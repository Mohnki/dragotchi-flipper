...
GameEventFlags advance_hunger(struct GameState *gs, uint32_t now) {
    ...
    uint32_t events = (now - p->last_hunger_update) / HU_DECAY_FREQ;
    p->last_hunger_update += events * HU_DECAY_FREQ;
...
GameEventFlags advance_happiness(struct GameState *gs, uint32_t now) {
    ...
    uint32_t events = (now - p->last_happiness_update) / HAP_DECAY_FREQ;
    p->last_happiness_update += events * HAP_DECAY_FREQ;
...
GameEventFlags advance_health(struct GameState *gs, uint32_t now) {
    ...
    uint32_t events = (now - p->last_health_update) / HP_CHECK_FREQ;
    p->last_health_update += events * HP_CHECK_FREQ;
    bool bad = (p->hunger == 0) || p->sick || (p->poop > 0);
    GameEventFlags flags = EVT_NONE;
    while(events-- > 0 && p->stage != DEAD) {
        if(bad) {
...
void hunger_feed(struct GameState *gs) {
    ...
    if(p->hunger >= MAX_HU) {
        care_penalty(p, CARE_OVERFEED); // overfeeding a full pet
        return;
    }
